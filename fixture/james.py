...
class JamesHelper:
    ...
    class Session:
        ...
        def __init__(self, host, port, username, password):
            ...
            self.write(username + "/n")
            self.read_until("Password:")
            self.write(password + "/n")
        ...
        def is_users_registred(self, username):
            self.telnet.write("verify %s/n" % username)
            res = self.telnet.expect([b"exists", b"does not exist"])
            return res[0] == 0

        def create_user(self, username, password):
            self.write("adduser %s %s/n" % (username, password) + "/n")
            self.read_until("User %s added" % username)

        def reset_password(self, username, password):
            self.write("setpassword %s %s/n" % (username, password) + "/n")
